...
## Deploying to Vercel
- The included `vercel.json` is a Vercel v2 config (explicit `"version": 2`) so deployments use the modern runtimes and avoid legacy runtime errors such as `now-php` version warnings. All serverless functions use the Node 20 runtime (`"runtime": "nodejs20.x"`). Traffic is rewritten to `api/server.js`, which re-exports the request handler from `server.js`, so Vercel uses the same code as local development without needing Express.
